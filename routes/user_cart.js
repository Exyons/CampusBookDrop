...
router.get("/user_cart/itemcount", deleteImages, wrapAsync(async (req, res) => {
    if (!req.user) {
        if (req.session.cart) {
...
router.patch("/user_cart/:id/update", wrapAsync(async (req, res) => {
    const { id } = req.params;
    const { q } = req.query;
    // console.log(q)
    const foundProduct = await Product.findById(id);
...
router.delete("/user_cart/destroy", wrapAsync(async (req, res) => {
    if (!req.user) {
...
router.delete("/user_cart/:id/remove", wrapAsync(async (req, res, next) => {
    // console.log(req.params);
    const { id } = req.params;
    // console.log(id);
    const foundProduct = await Product.findById(id);
