const getPoints = async () => {
    const saleSeries = { name: "Total Sale", points: [] }
    const earningSeries = { name: "Total Earnings", points: [] }
    try {
        const res = await axios.get("/user/books/analytics");
        saleSeries.points = res.data.saleSeriesPoints;
        earningSeries.points = res.data.earningSeriesPoints;
        // const point = res.data.points[0]
        // console.log(point);
        // console.log(new Date(point[0]).format("yyyy-mm-dd"));
        // console.log(new Date(point[0]).getFullYear());
        // console.log(new Date(point[0]).getMonth());
        return { saleSeries, earningSeries };
    } catch (error) {
        console.log(error)
...
getPoints()
    .then(result => {
        const totalProductsSoldChart = JSC.chart('sellerTotalProductsSoldChart', {
            debug: false,
            type: 'area',
            legend: {
                template: '%icon %name',
        ...
        const totalEarningsChart = JSC.chart('sellerTotalEarningsChart', {
            debug: false,
            type: 'area',
            legend: {
                template: '%icon %name',
